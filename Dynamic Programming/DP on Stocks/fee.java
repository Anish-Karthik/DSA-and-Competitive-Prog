class Solution {
    ...
    int f(int ind, int canSell, int[] prices) {
        ...
        if(canSell == 0) {
            ...
            profit = Math.max(+prices[ind] - fee + f(ind+1,0,prices), f(ind+1,canSell,prices));
        }
        ...
        for(int ind = n-1; ind>=0;ind--) {
            for(int canSell = 1; canSell>=0; canSell--) {
                ...
                if(canSell == 0) {
                    ...
                    profit = Math.max(+prices[ind] - fee + dp[ind+1][0], dp[ind+1][canSell]);
                }
                dp[ind][canSell] = profit;
            }
        }
    ...
    int so(int[] prices) {
        ...
        for(int ind = n-1; ind>=0;ind--) {
            ...
            cur[1] = Math.max(+prices[ind] - fee + next[0], next[1]);
            next = cur;
        }
        return next[0];
    }
    int sso(int[] prices) {
        ...
        for(int ind = n-1; ind>=0;ind--) {
            ...
            curSell = (Math.max(+prices[ind] - fee + nextNotSell, nextSell));
